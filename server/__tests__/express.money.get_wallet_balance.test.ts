...
import get_wallet_balance from '../src/routes/money/get_wallet_balance';
...
describe('Test the get_wallet_balance function', () => {
  ...
  test('valid user_id should return 1250', () => {
    get_wallet_balance(req, res)
    expect(res.statusCode).toBe(200);
    expect(res.text).toBe(250); // test user has 1000 in bank and 250 in wallet
  });

  test('invalid user_id should return error', () => {
    get_wallet_balance({ params: { user_id: "invalid uuid" } }, res);
    expect(res.statusCode).toBe(400);
    expect(res.text).toBe('Invalid user_id');
  });

  test('non-existant user_id should return error', () => {
    get_wallet_balance({ params: { user_id: "bb4d7f07-b82a-4b3b-bbf3-382d431b907b" } }, res);
    expect(res.statusCode).toBe(400);
